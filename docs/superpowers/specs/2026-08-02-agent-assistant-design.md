...
| 组件 | 职责 | 依赖 |
| --- | --- | --- |
| `intent.py` | 先查**意图识别缓存**（仅缓存识别结果，短 TTL，不缓存业务查询），未命中再调 LLM 抽取意图（JSON 校验，失败重试一次），LLM 不可用/超时退回关键词/正则；均失败返回引导回复 | `llm.py` |
...
| `llm.py` | 无状态，不直接访问业务库；调用 Ollama `/api/chat`（`format: json` 用于意图抽取）；含超时（10s）、并发上限、连续失败熔断与降级信号 | Ollama HTTP |
...
- 按用户维护**会话历史**：消息列表滑动窗口（默认最近 20 条 / 约 8k token，超出丢弃最旧）；
- 同时维护**事实表**：只记影响后续对话的稳定事实（如"10:05 已选课：高等数学""10:06 已退课：线性代数"），用于"把刚才选的课退掉"这类指代；
- LLM 上下文 = 最近消息窗口 + 事实表（压缩后），不发送全量历史；
- 会话 TTL：30 分钟无活动自动清空；服务重启即丢失（v1 明确不持久化）。
