    ...
    <el-dialog v-model="dialogVisible" title="提交请假申请" width="480px">
      <div class="content-card">
        <el-form :model="form" label-width="80px">
          <el-form-item label="开始日期"><el-date-picker v-model="form.start_date" type="date" value-format="YYYY-MM-DD" placeholder="选择开始日期" /></el-form-item>
          <el-form-item label="结束日期"><el-date-picker v-model="form.end_date" type="date" value-format="YYYY-MM-DD" placeholder="选择结束日期" /></el-form-item>
...
async function submitApply(){ if(!form.value.start_date||!form.value.end_date||!form.value.reason){ElMessage.warning('请填写完整');return} submitting.value=true; try{await applyLeave(form.value);ElMessage.success('已提交');dialogVisible.value=false;form.value={start_date:'',end_date:'',reason:''};load()}catch(e){ElMessage.error(e?.response?.data?.detail||'提交失败')}finally{submitting.value=false} }
