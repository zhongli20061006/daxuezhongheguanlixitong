...
async function submitApply(){ if(!form.value.start_date||!form.value.end_date||!form.value.reason){ElMessage.warning('请填写完整');return} submitting.value=true; try{const data={start_date:form.value.start_date,end_date:form.value.end_date,reason:form.value.reason};await applyLeave(data);ElMessage.success('已提交');dialogVisible.value=false;form.value={start_date:'',end_date:'',reason:''};load()}catch(e){ElMessage.error(e?.response?.data?.detail||'提交失败')}finally{submitting.value=false} }
