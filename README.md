...
FastAPI + Vue 3 全栈智慧校园平台，内置本地 AI 智能体（首页即对话窗口），涵盖选课、成绩、课表、请假、毕业审核、报修、通知等功能。
...
种子账号密码由初始化脚本统一设置并打印（`python -m app.init_data` 或 docker-init 的运行输出），不在此记录明文：
...
| 退掉人工智能实战 | 同样需要二次确认 |
| 查询第三周周一1-2节的空教室 | 直接返回该时段空闲教室列表（排除课程占用与已预约），可接着说"预约 D101 第3周周一1-2节"走预约流程 |
...
| 查一下2024级计算机科学1班的课表 | 教师按班级返回课表卡片 |
| 查一下5班有哪些学生 | 教师按班级/姓名/学号返回学生名单卡片 |
| 把张三的高数成绩录成90分 | 教师录入成绩，预检后弹出确认卡片，确认后落库并自动计算总评 |
| 查我的监考安排 | 教师返回监考安排卡片 |

**教师端**：教师登录后，首页快捷提示自动切换为教师工作流（录成绩 / 查班级课表 / 查学生名单 / 查我的监考）。成绩录入仅限教师角色，且只能录入自己授课课程的学生成绩；确认后自动计算总评（平时 20% + 期末 80%）。
...
- **多轮补全**：写操作参数不全时（如请假缺日期），下一条消息补充信息（"明天到后天，感冒"）自动合并继续，再回复"确认/下一步"即执行
- **LLM 输出兜底校验**：模型把动作请求误判成闲聊、或返回非法跳转（如"下一步"→next）时，自动回退规则引擎，避免代写文案/空跳转
- **课程模糊匹配**：说"选摄影课"这类简称/口语名时，系统会从该班级课表里按"精确 > 包含 > 公共子串"选最贴合课程；匹配不到或有多门时列出可选课程提示确认
- **日期宽容解析**：请假支持"今天/明天/后天"、"8.3号-8.5号"、"8月4日"等自然写法；写操作参数以规则抽取为准，避免模型幻觉旧日期
- **学习方案完整性**：模型漏课（如只给最高优先级科目）时自动用模板补齐所有科目；追问"其他科目的呢/剩下的呢"会重新生成完整方案
...
| POST /agent/confirm | 确认写操作（选课 / 退课 / 请假 / 报修 / 教室预约 / 审批请假 / 成绩录入） |
| GET /agent/status | Ollama 在线状态 + 熔断状态 |
| GET /agent/sessions | 会话列表 |
| GET / DELETE /agent/sessions/{id} | 会话历史 / 删除会话 |

**当前状态**：跳转、查课表、学习方案、选课、退课、查教室、请假、报修、教室预约、查成绩、查通知、审批请假、查考试/监考已全部接入真实业务 API（写操作含预检与二次确认）。教师端：按班级查课表、查学生名单、查我的监考、成绩录入（仅限本人授课课程，写操作二次确认）。
